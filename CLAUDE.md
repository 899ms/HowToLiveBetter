...
- 条目数、A 级条数变了要同步四处：README（正文数字与两个徽章）、index.html（五处描述、numberOfPages、页头条目数）、CLAUDE.md 的目录简介，以及 **tools/og.html + 重新截图 og.png**（命令写在 og.html 文件头注释里，链接数的口径是 book/ 下所有「- 来源：」和「- 备注：」行里的 http(s) 总数）。最后这处最容易漏。**但这四处不要每轮改完就同步**（2026-09-18 用户定：「你不要每次都改 OG 什么的，可能我一轮没加完呢，等我说改完了再改，不然浪费 token」）：一轮里可能连着加好几条，每次都重算数字、重跑 Chrome 截图再 Read 一遍图是白烧 token。做法是正文改完先只报一句「条目数 X→Y、A 级 X→Y、链接 X→Y 待同步」，等用户说改完了再一次性对齐五处并重出 og.png。**本仓库以外的地方一律不写条目数**（组织主页 `D:\projects\eternity4719-profile` 的自述、对外介绍等），写了就等于多一处要同步、迟早过期；那些地方只写这本书是什么。
...
3. 不要浪费精力（睡眠、争吵、信息噪音、多任务、决策疲劳、人际负债、把机构当成按规则上班的人而不是角色——总纲条，实操分别在第 8、24 节）
