...
电子版三样，全由 `.github/workflows/book.yml` 在 main 上正文改动后自动生成，挂到固定 Release `epub-latest`（下载链接不变，README 引用的就是它），PR 只生成校验不发布；产物都进 `dist/`（已 gitignore）。**产物本身不入库**，正文几乎天天改，提交二进制只会过时和撑大历史（2026-09-18 issue #12 定的方案）。README 的结构解析和文件清单收在 `tools/lib/book.mjs`，三套构建共用，新增节或长文不用改脚本。**但新增 docs 长文必须手工挂三处链接**（2026-09-21 漏过一次，用户在检索页上发现的）：① README 末尾「长文另见」那句；② README 对应节的目录行（写成「长文见 [docs/x.md](docs/x.md)。」）；③ index.html 里 `class="doc-links"` 那一行，URL 要百分号编码。第 ① 处是硬性的——`book.mjs` 第 54 行靠正则 `\]\((docs/[^)#/]+\.md)\)` 从 README 里扒长文清单，只在 book/ 条目里链接，EPUB、PDF、离线单文件三套构建都收不到它。sync-stats.ps1 不检查这三处。
