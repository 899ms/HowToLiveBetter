...
- 备注：吸别人的烟同样会死人。2004 年全球约有 60.3 万人死于二手烟。这占当年全球总死亡的约 1%。自己不吸，也要躲开别人的烟。尤其别让孩子吸到。这两个数字出自 Oberg M 等 (2011). Worldwide burden of disease from exposure to second-hand smoke: a retrospective analysis of data from 192 countries. Lancet. <https://doi.org/10.1016/S0140-6736(10)61388-8>
...
- 备注：从 4000 步加到 7000 步这一段最划算，不必非要凑一万步。要注意这类研究只是跟踪记录：走得最少的那批人里，混着本来就病着走不动的。他们是快不行了才走得少，不是因为少走才早死。所以算出的差距常常偏大。但越走越好的趋势是清楚的。这一条和第 10 条（每周 150–300 分钟中等强度运动）说的是同一件事，只是换了个数法，达标一个就行。
...
- 备注：这一条和第 7 条（每天走到 7000–8000 步）挑一个做就行。要注意这类研究只是跟踪记录。用仪器实测活动量的那几项，跟踪时间短，参与者偏老，数字里混着「先病了才不动」的成分。真实好处没有「风险降到 0.52 倍」那么大。
...
- 备注：争议在三点。一是这套分类（NOVA）把营养差别很大的食物划成了一类。二是它和含糖饮料、加工肉（第 3、15 条）大量重叠，分不清到底是哪一项在起作用。三是按通行的证据打分法（GRADE）评，等级也低。所以做到那两条之后，这一条还能多带来多少好处，说不准。反方的证据就是同一篇综述给出的低评级本身，目前没有得出相反结论的原始研究。所以先把第 3、15 条（不喝含糖饮料、少吃加工肉）做到，再看这一条。
